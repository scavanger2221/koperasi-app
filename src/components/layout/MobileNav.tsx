...
import { Home, Users, Wallet, FileText, Menu } from 'lucide-react'
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--color-border)] z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      <div className="flex items-center justify-around h-16">
        ...
        <button
          onClick={onMenuOpen}
          className="flex flex-col items-center justify-center flex-1 h-full text-[var(--color-text-soft)] px-1"
        >
          <Menu className="w-6 h-6 shrink-0 mb-1 stroke-2" />
          <span className="text-xs font-medium truncate w-full max-w-full text-center leading-none">
            Menu
          </span>
        </button>
      </div>
